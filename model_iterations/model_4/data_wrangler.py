...
class Wrangler:
    ...
    def load_and_transform_imgs(self, path_list):
        ...
        graycale_images = (images * gray_weights).sum(dim=1, keepdim=True)  # broadcast gray weights across channels, sum channels --> single channel

        threshold = 0.9                                                    # threshold is 90% pixel intensity
        binary_images = (graycale_images > threshold).float()               # grayscale --> binary img w/ thresholding

        return labels, binary_images
